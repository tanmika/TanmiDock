...
## [1.0.7-hotfix1] - 2026-08-23

### Fixed

- 基于 `1.0.3` 修复 CodePac action 显式空 `--targetdir` 的目录语义：递归配置继续从 `--configdir` 指定目录读取，依赖生成到父级目标目录，恢复 Web 预设库 715 分支要求的顶层 `3rdparty` 平铺结构。
- `link`、`status`、`reset`、`unavailable` 统一使用“省略、显式空值、显式非空值”三种目标目录状态，并为每一级 action 增加输入、解析结果、最终目录和异常分支日志。
- 依赖图按“配置文件与目标目录”组合记录访问状态，保证同一递归配置被多个目标目录引用时能够分别处理。
- `unavailable` 的依赖图调试日志增加最终依赖数量、配置数量和去重后依赖清单，便于完整判断递归处理结果。

### 发布范围

- 本紧急修复版本从 `1.0.3` 单独演进。
- `1.0.4` 至 `1.0.6-beta2` 中仍待重新验证的能力不包含在本版本中；`1.1.0` 开发分支继续独立演进，后续仅同步本次修复提交。
